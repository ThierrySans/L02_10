 CREATE OR REPLACE FUNCTION Add_Answer(Users varchar(100), assignmentID int , questionId int, answer varchar(255))
 	...
 	BEGIN
	 	IF EXISTS (SELECT 1 FROM UserAssignQuesAnsMap WHERE UTOR_Id=Users AND Assignment_Id=assignmentID AND Question_Id=questionId) THEN
 			UPDATE UserAssignQuesAnsMap SET Answer = answer, Temp_Answer = NULL WHERE UTOR_Id=Users AND Assignment_Id=assignmentID AND Question_Id=questionId;
 			RETURN 'Offical Answer Modified';
 		ELSE
		  	INSERT INTO UserAssignQuesAnsMap (UTOR_Id, Assignment_Id,Question_Id,Answer,Temp_Answer) VALUES (Users, assignmentID,questionId,answer,NULL);
